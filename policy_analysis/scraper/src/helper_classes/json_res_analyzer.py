...
class JSONAnalyzer:
    ...
    def get_email_status(self, privacy_policy_url):
        for obj in self.data:
            if privacy_policy_url in obj:
                ...
                return bool(policy_data.get("emails"))
        return False
    ...
    def get_phonenumber_status(self, privacy_policy_url):
        for obj in self.data:
            if privacy_policy_url in obj:
                policy_data = obj[privacy_policy_url]
                return bool(policy_data.get("phonenumbers"))
        return False

    def _extract_keys(self):
        return {list(obj.keys())[0] for obj in self.data if obj}

    def save_url_keys(self, output_txt_file):
        with open(output_txt_file, "w", encoding="utf-8") as file:
            for url in sorted(self.url_keys):
                file.write(url + "\n")
...
if __name__ == "__main__":
    csv_file = "results/data_source_with_ppurl.csv"
    json_file = "results/scraped_results.json"
